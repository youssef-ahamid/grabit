...
>
  ...
  >
    <div class="flex space-x-4 px-4 items-center">
      <div class="p-4 rounded-full bg-brand">
        <Icon
          name="emailAttachment"
          className="w-6 text-neutralLight"
        />
      </div>
      <Go to="mailto:{email}">
        <p
          class="font-bold hover:text-brand slide-underline transition-300"
        >
          {email}
        </p>
      </Go>
    ...
    <div
      class="h-[70%] w-px bg-neutralDark absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2"
    />
  </div>
  <footer
    class={`${classes.footer} max-w-[80%] mx-auto ${className}`}
  >
    <div
    ...
    >
      ...
      <div class="flex space-x-2">
        {#each socials as social, i}
          {#if social}
            <Animateonenterview type="flyUp" delay={200 + 200 * i}>
              <Go to={social.link} redirect>
    ...
    <p
      class="font-medium text-xs text-neutral-500 absolute right-2 bottom-2 text-neutralLight"
    >
      {copyright}
    </p>
  </footer>
