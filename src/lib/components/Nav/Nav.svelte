...
<div
  class="z-[998] ring-1 fixed w-12 bg-brandDark left-0 top-0 bottom-0 transition-300 max-w-[100vw] max-h-[100vh] overflow-hidden ring-neutral"
>
  <div
    class="relative h-screen w-12"
    use:clickOutside
    on:outclick={() => {
  ...
  >
    ...
    <Go to="/">
      <img
        {...logoSmall}
        class="right-1/2 translate-x-1/2 absolute bottom-1/2 translate-y-1/2"
      />
    </Go>

    <Go
      to="tel:+201023680518"
      className="-rotate-90 bottom-20 absolute right-1/2 translate-x-1/2"
    >
      <Button
        label="contact us"
        reverse
...
<div class="relative h-8 -mt-8 pt-8 max-w-[100vw] overflow-hidden">
  <nav class={`${classes.nav} ${className}`}>
    <div class="relative w-full h-full">
      <div
        class="flex flex-col text-neutral items-stretch md:justify-between h-screen md:pt-20"
      >
        <div class="">
          <List
          ...
          >
            ...
            >
              ...
              >
                <p
                  class="text-neutral text-sm font-bold group-hover:text-brandLight transition-300 max-w-fit whitespace-nowrap"
                >
                  {link.text}
                </p>
                <ChevronRight
                  className="w-6 md:w-8 text-neutral group-hover:text-brandLight group-hover:translate-x-2"
                />
              </Go>
            </Animateonenterview>
          </List>
        ...
        <div class="md:px-4 md:pt-8">
          ...
          >
            <Animateonenterview
              type="flyUp"
              className="-my-1 md:my-0 py-0 pr-1"
              delay={links.filter(link => link.group === 'info')
                .length *
                100 +
                100 * index}
            >
