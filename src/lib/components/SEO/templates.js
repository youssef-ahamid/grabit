export const article = blog => {
    const { image, slug, title, otherTitle, createdAt, updatedAt, author, keywords, genre, body } = blog
    let obj = {
...
export const getTemplate = (name, data) => {
    ...
    if(!template) return console.error(`template with name ${name} not found`);
    return `<script type="application/ld+json">${template.generate(data)}</script>`
}